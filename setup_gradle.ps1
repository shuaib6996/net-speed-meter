# PowerShell script to setup Gradle wrapper for NetSpeed Meter project
...
$wrapperUrl = "https://github.com/gradle/gradle/raw/v8.4/wrapper/gradle-wrapper.jar"
...
if (-not (Test-Path $wrapperJar)) {
    ...
    try {
        ...
        Write-Host "Please download it manually from: $wrapperUrl" -ForegroundColor Yellow
    }
}

# Create gradlew script for Unix (simplified)
$gradlewContent = @'
#!/usr/bin/env bash
...
if ($IsWindows) {
    # On Windows, we need to make gradlew executable
    # This is just for compatibility, we'll use gradlew.bat
    Write-Host "Created gradlew script for Unix systems." -ForegroundColor Green
}

Write-Host "Gradle wrapper setup complete." -ForegroundColor Green
Write-Host ""
Write-Host "To build the APK, run:" -ForegroundColor Cyan
Write-Host "  .\gradlew.bat assembleDebug" -ForegroundColor White
Write-Host ""
Write-Host "The APK will be created at:" -ForegroundColor Cyan
